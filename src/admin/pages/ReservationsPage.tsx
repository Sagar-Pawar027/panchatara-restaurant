...
export function ReservationsPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await getReservations(statusFilter, dateFilter || undefined);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed creating reservation:', err);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Reservations List */}
      {loading ? (
        <div className="py-20 text-center text-xs text-white/40">Loading reservations...</div>
      ) : filtered.length === 0 ? (
