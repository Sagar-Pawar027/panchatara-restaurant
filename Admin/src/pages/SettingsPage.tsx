...
import { getAdminStats, updateSettings, reconnectDatabase } from '../services/api.ts';
import { SettingsShimmer } from '../components/AdminShimmer.tsx';
...
export function SettingsPage() {
  ...
  useEffect(() => {
    if (stats?.settings) {
      setRestaurantName(stats.settings.restaurantName || 'Panjtara Pure Veg');
      setTagline(stats.settings.tagline || '');
      setPhone(stats.settings.phone || '+91 95220 10107');
      setAddress(stats.settings.address || '');
      setOpeningHoursLunch(stats.settings.openingHoursLunch || '11:00 AM - 03:30 PM');
      setOpeningHoursDinner(stats.settings.openingHoursDinner || '06:30 PM - 12:00 Midnight');
      setAcceptingReservations(stats.settings.acceptingReservations ?? true);
      setNoticeBanner(stats.settings.noticeBanner || '');
    }
  }, [stats]);
  ...
  const databaseType = stats?.databaseType || 'Memory / Offline Mock';
  const databaseConnected = stats?.databaseConnected ?? false;
  const dbStatus = stats?.databaseStatus || null;
  const reconnecting = reconnectMutation.isPending;
